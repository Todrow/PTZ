...
class ExcelWrapper: # 
    ...
    def formatTitles(self, ws, do_add: bool) -> None:
        ...
        names = ["Модель трактора", "№ трактора", "Граничная дата гарантии", "Продолжительность контроля, м/ч", "Наработка, м/ч",
                 "Опытный узел", "Дата и время обращения", "ПЭ: Комментарий", "Дефект выявлен на м/ч", "Разработчик программы ПЭ"]
        if do_add:
            ws.insert_rows(0)
            names = ["Модель трактора", "№ трактора", "Опытный узел", "Дата и время обращения",
                     "Продолжительность контроля, м/ч", "Наработка, м/ч", "Дефект выявлен на м/ч", "ПЭ: Комментарий", "Граничная дата гарантии", "Разработчик программы ПЭ"]
    ...
    def formattingCells(self, ws) -> None: 
        ...
        widths = {'A': 17, 'B': 20, 'C': 53.441, 'D': 18.664, 'E': 23.109,
                  'F': 12.886, 'G': 20.332, 'H': 105.441, 'I': 19.109, 'J': 25.441}
        for row in ws.rows:
            for cell in row:
